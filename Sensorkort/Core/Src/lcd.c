...
/**
 @brief send_display_data, setup of display bitstream
 @param rw, R/W bit according to data sheet
 @param rs, RS bit according to data sheet
 @param data data byte to be sent to display
 @return void
 */
void lcd_send_data_1(uint8_t rw, uint8_t rs, uint8_t data){

	//SID Input bitstream to be set: 1111 1RWRS0 D0D1D2D3 0000 D4D5D6D7 0000

	uint8_t startByte = 0x1F; 		//Byte(LSB): 1111 1000
	uint8_t firstByte = 0x00;		//Byte(LSB): 0000 0000
	uint8_t secondByte = 0x00;		//Byte(LSB): 0000 0000

	rw = rw << 5;					//Byte(LSB): 0000 0RW00
	startByte = startByte | rw;		//Byte(LSB): 1111 1RW00
	rs = rs << 6;					//Byte(LSB): 0000 00RS0
	startByte = startByte | rs;		//Byte(LSB): 1111 1RWRS0
	start = startByte;

	firstByte = firstByte | (data & 0x0F);	//firstByte: D0D1D2D3 0000
	first = firstByte;

	data = data & 0xF0;
	data = data >> 4;
	secondByte = secondByte | data;			//secondByte: D4D5D6D7 0000
	second = secondByte;
	...
	HAL_GPIO_WritePin(GPIOA, Disp_Reset_Pin , GPIO_PIN_SET);
	HAL_SPI_Transmit(&hspi2, buffer, 3, 1000);
	HAL_GPIO_WritePin(GPIOA, Disp_Reset_Pin , GPIO_PIN_RESET);
}
